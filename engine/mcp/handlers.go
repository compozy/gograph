...
func (s *Server) HandleGetPackageStructureInternal(ctx context.Context, input map[string]any) (*ToolResponse, error) {
	...
	query := `
		MATCH (pkg:Package {project_id: $project_id})
		WHERE pkg.import_path = $package OR pkg.name = $package
...
func (s *Server) HandleVerifyCodeExistsInternal(ctx context.Context, input map[string]any) (*ToolResponse, error) {
	...
	case "package":
		query = `MATCH (p:Package {project_id: $project_id}) WHERE p.import_path = $name OR p.name = $name RETURN p, true as has_file`
	default:
